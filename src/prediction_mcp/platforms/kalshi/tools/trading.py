...
async def batch_cancel_orders(order_ids: Optional[List[str]] = None, ticker: Optional[str] = None) -> Dict:
    """Cancel multiple orders at once by IDs or ticker."""
...
async def get_orders(ticker: Optional[str] = None, status: Optional[str] = None, limit: int = 100) -> List[Dict]:
    """List orders with optional filters for ticker and status."""
...
async def get_fills(ticker: Optional[str] = None, limit: int = 100) -> List[Dict]:
    """Get order execution history with fill prices and timestamps."""
...
async def simulate_order(ticker: str, side: str, action: str, count: int, price: int) -> Dict:
    """Dry-run an order to preview cost and slippage without placing it."""
...
async def get_order_history(ticker: Optional[str] = None, limit: int = 100) -> List[Dict]:
    """Retrieve closed order history with final status."""
    return await get_client().get_orders(ticker=ticker, status="closed", limit=limit)

def get_tools() -> List[types.Tool]:
    return [
        types.Tool(name="kalshi_create_order", description="Place an order to buy or sell Yes/No contracts on a market. Specify ticker, side (yes/no), action (buy/sell), count (number of contracts), order type (limit/market), and price (1-99 cents, required for limit orders).", inputSchema={"type": "object", "properties": {"ticker": {"type": "string"}, "side": {"type": "string", "enum": ["yes", "no"]}, "action": {"type": "string", "enum": ["buy", "sell"]}, "count": {"type": "integer"}, "price": {"type": "integer"}, "order_type": {"type": "string", "enum": ["limit", "market"], "default": "limit"}}, "required": ["ticker", "side", "action", "count"]}),
